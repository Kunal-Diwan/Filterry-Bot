from FilterryBot.modules.sql.translation import switch_to_locale, prev_locale
from FilterryBot.modules.translations.strings import tld
from telegram.ext import CommandHandler
from telegram import ParseMode, InlineKeyboardMarkup, InlineKeyboardButton
from FilterryBot import dispatcher
from FilterryBot.modules.translations.list_locale import list_locales
...
def locale(bot, update, args):
    ...
    if len(args) > 0:
        ...
        if locale in list_locales:
            if locale in ('en', 'ru', 'ua', 'es', 'hi', 'id'):
...
def locale_button(bot, update):
    ...
    lang_match = re.findall(r"en|ru|ua|es|hi|id", query.data)
    if lang_match:
        if lang_match[0]:
